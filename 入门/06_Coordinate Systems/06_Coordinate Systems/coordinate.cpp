...
#define STB_IMAGE_IMPLEMENTATION //预处理器会修改头文件，只应用函数定义的源码
#include "stb_image.h"
#include "shader_m.h"
...
int main() {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//model = glm::translate(model, modelPosition + modelOffset);
		...
		// pass them to the shaders (3 different ways)三种不同的方式给Shader中的MVP矩阵赋值
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	//modelOffset = glm::vec3(front.x, 0.0f, front.y);
}